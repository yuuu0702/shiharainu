...
class AppTheme {
  ...
  static const Color destructiveColor = Color(
    0xFFD4183D,
  ); // Destructive: #d4183d
  ...
  static const Color foregroundColor = Color(0xFF000000); // Black text
  ...
  static ThemeData get lightTheme => ThemeData(
    ...
    chipTheme: ChipThemeData(
      ...
      labelStyle: const TextStyle(color: mutedForeground, fontSize: 12),
  ...
  static const double focusBorderWidth = 3.0; // フォーカス時のボーダー幅
  static const double minimumTouchTarget = 44.0; // 最小タッチターゲットサイズ（WCAG準拠）

  // フォーカスデコレーション取得メソッド
  static BoxDecoration getFocusDecoration({double? borderRadius}) {
    return BoxDecoration(
      border: Border.all(color: focusBorderColor, width: focusBorderWidth),
      borderRadius: BorderRadius.circular(borderRadius ?? radiusSmall + 2),
    );
  }

  // アクセシブルなフォーカス表示（アウトライン）
  static BoxDecoration getAccessibleFocusDecoration({double? borderRadius}) {
    return BoxDecoration(
      border: Border.all(color: focusBorderColor, width: focusBorderWidth),
      borderRadius: BorderRadius.circular(borderRadius ?? radiusSmall + 2),
      boxShadow: [
        BoxShadow(
          color: focusBorderColor.withValues(alpha: 0.25),
          blurRadius: 4,
          spreadRadius: 0,
