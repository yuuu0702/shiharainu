...
/// イベントコレクション参照プロバイダー
final eventsCollectionProvider = Provider<CollectionReference<EventModel>>((ref) {
...
final eventDocProvider = Provider.family<DocumentReference<EventModel>, String>((ref, eventId) {
  final eventsCollection = ref.watch(eventsCollectionProvider);
  return eventsCollection.doc(eventId);
});

/// 参加者サブコレクション参照プロバイダー（ファミリー）
final participantsCollectionProvider = Provider.family<CollectionReference<ParticipantModel>, String>((ref, eventId) {
  final firestore = ref.watch(firestoreProvider);
  return firestore
      .collection('events')
      .doc(eventId)
      ...
      .withConverter<ParticipantModel>(
        fromFirestore: (snapshot, _) => ParticipantModel.fromFirestore(snapshot),
        toFirestore: (participant, _) => ParticipantModel.toFirestore(participant),
      );
});

/// 支払いサブコレクション参照プロバイダー（ファミリー）
final paymentsCollectionProvider = Provider.family<CollectionReference<PaymentModel>, String>((ref, eventId) {
  final firestore = ref.watch(firestoreProvider);
  return firestore
      .collection('events')
      .doc(eventId)
      ...
      .withConverter<PaymentModel>(
        fromFirestore: (snapshot, _) => PaymentModel.fromFirestore(snapshot),
        toFirestore: (payment, _) => PaymentModel.toFirestore(payment),
      );
});

/// 通知コレクション参照プロバイダー
final notificationsCollectionProvider = Provider<CollectionReference<NotificationModel>>((ref) {
  final firestore = ref.watch(firestoreProvider);
  return firestore.collection('notifications').withConverter<NotificationModel>(
    fromFirestore: (snapshot, _) => NotificationModel.fromFirestore(snapshot),
    toFirestore: (notification, _) => NotificationModel.toFirestore(notification),
  );
});
...
class FirestoreService {
  ...
  /// 招待コードからイベントを検索
  Future<DocumentSnapshot<Map<String, dynamic>>?> findEventByInviteCode(String inviteCode) async {
